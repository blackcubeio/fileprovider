...
class ResumableService
{
    ...
    public function saveChunk(
        ...
        if ($this->fileProvider->directoryExists($chunkDir) === false) {
    ...
    public function assemble(string $identifier, string $filename): string
    {
        ...
        if ($assembled !== false) {
            ...
            foreach ($listing as $item) {
                if ($item->isFile() === true && str_starts_with(basename($item->path()), $chunkPrefix) === true) {
                    $chunks[] = $item->path();
                }
            }
            natsort($chunks);
            ...
            foreach ($chunks as $chunkPath) {
                ...
                if (is_resource($chunkStream) === true) {
    ...
    {
        if ($this->fileProvider->isTempPath($name) === false) {
            throw new \InvalidArgumentException('Can only delete temporary files');
        }

        if ($this->fileProvider->fileExists($name) === true) {
            $this->fileProvider->delete($name);
        }
    }

    /**
     * Check if a file exists on a filesystem this provider handles.
     */
    public function fileExists(string $name): bool
    {
        return $this->fileProvider->canHandle($name)
            && $this->fileProvider->fileExists($name) === true;
    }
    ...
    public function getPreview(string $name, bool $original = false): ?array
    {
        ...
        if ($this->fileExists($name) === true) {
            $mimeType = $this->fileProvider->mimeType($name);
            $filename = basename($name);
            $needsThumbnail = $original === false && $this->isImage($name) === true;

            $stream = $needsThumbnail
                ? $this->fileProvider
                    ->cover($this->config->getThumbnailWidth(), $this->config->getThumbnailHeight())
                    ->readStream($name)
    ...
    public function isImage(string $name): bool
    {
        ...
        return in_array($extension, self::IMAGE_EXTENSIONS, true) === true;
    }
    ...
    {
        $extension = strtolower(pathinfo($name, PATHINFO_EXTENSION));
        return in_array($extension, self::SVG_EXTENSIONS, true) === true;
    }

    /**
     * Temporary filesystem alias, sourced from the FileProvider (single source).
     */
